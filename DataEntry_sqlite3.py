...
# from Sqlite3.connector import Error
import os
...
class dataentry(tk.Tk):
    def __init__(self, filepath):
        ...
        try:
            ...
            print("Error while connecting to Sqlite3", e)
        ...
        def select_key(event):
            ...
            return
            for itm in range(self.List_Prem_Province.size()):

                # print(self.List_Prem_Province.get(itm)[:1])
                if event.keysym.upper() == self.List_Prem_Province.get(itm)[:1]:
                    self.List_Prem_Province.select_set(itm)
        ...
        self.List_Prem_Province.bind('<KeyPress>', select_key)
        ...
        Religion_list = ['Islam', 'Christanity', 'Hindu', 'Other']
        ...
        self.List_Approver.insert(2, 'AC (Rural)')
        self.List_Approver.grid(
            column=3, row=12, padx=10, pady=10, sticky=tk.W)
        # Check Buttons
        Approver_List = ['AC (Saddar)', 'AC (I/A)',
        ...
        def edit():
            today = date.today()
            validatin_result = Validation.validate_CNIC(self.Entry_CNIC.get())
            if validatin_result != 'valid':
                messagebox.showerror(
                    'showerror', validatin_result)
            ...
            try:
                ...
                if len(data) != 0:
                    for row in data:
                        ...
                        self.Entry_First_Name.insert(0, row['First_Name'])
                        self.Entry_Last_Name.delete(0, 'end')
                        self.Entry_Last_Name.insert(0, row['Last_Name'])
                        ...
                        Tehsil_values = list(self.Tehsil_data_dict.values())
                        District_values = list(
                            self.District_data_dict.values())
                        Tehsil_txt = Tehsil_List[Tehsil_values.index(
                            row['Pres_Tehsil'])]
                        District_txt = District_List[District_values.index(
                            row['Pres_District'])]
                        self.Entry_Pre_Tehsil.delete(0, 'end')
                        ...
                        self.List_Pres_Province.select_set(
                            Province_Keys.index(row['Pres_Province']))
                        ...
                        print(Province_Keys.index(int(row['Perm_Province'])))
                        ...
                        self.List_Religion.select_set(
                            int(Religion_list.index(row['Religon'])))
                        ...
                        self.List_Process_Type.select_set(
                            Process_List.index(row['Process_Type']))

                        self.List_Request_Type.select_set(
                            int(row['Request_Type'])-1)
                        ...
                        self.List_Approver.select_set(
                            int(Approver_List.index(row['Approver_Desig'])))

                        # Check Buttons

                        cnic_front.set(row['cnic_front'])
                        cnic_back.set(row['cnic_back'])
                        cnic_guardian.set(row['cnic_guardian'])
                        form_b.set(row['form_b'])
                        domicile_of_guardian.set(row['domicile_of_guardian'])
                        noc_from_concerned_district.set(
                            row['noc_from_concerned_district'])
                        Residance_Prof.set(row['Residance_Prof'])
                        utility_bill.set(row['utility_bill'])
                        educational_certificate.set(
                            row['educational_certificate'])
                        marriage_registration_certificate.set(
                            row['marriage_registration_certificate'])
                        affidavit_voterlist.set(row['affidavit_voterlist'])
                        affidavit_domicile.set(row['affidavit_domicile'])
                        voter_list.set(row['voter_list'])
                        domicile_challan.set(row['domicile_challan'])
                else:
            ...
            try:
                ...
                if len(self.child_data_list) != 0:
                    for items in self.child_data_list:
                        ...
                        self.cur.execute("Insert into Childern (Father_CNIC, Child_Name, Child_dob) values ('{}', '{}', '{}')".format(
        ...
        def export_form():
            ...
            date_label = ttk.Label(
                export_window, text='Date', font=('Bell', 12))
            date_label.place(x=50, y=50)
            date_entry = ttk.Entry(export_window, font=('Bell', 12))
            ...
            date_button = Button(
                export_window, text='Export Data', width=10, font=('Bell', 12), command=export)
            date_button.place(x=100, y=100)
        ...
        self.Exit_button = tk.Button(
            self.Status_Frame, width=15, text='Edit', command=edit, font=('Bell', 12))
        self.Exit_button.grid(column=7, row=1, padx=10, pady=10)
        self.Export_button = tk.Button(
            self.Status_Frame, width=15, text='Export Data', command=export_form, font=('Bell', 12))
        self.Export_button.grid(column=9, row=1, padx=10, pady=10)
...
class NOC(tk.Tk):
    def __init__(self, filepath):
        ...
        self.Top_Frame.pack(fill=X)
        self.Top_label = ttk.Label(
            self.Top_Frame, text='NOC to Other Districts', border=1, font=('Bell', 18, 'bold'))
        self.Top_label.pack(padx=10, pady=10, side=TOP)
        self.Middle_Frame = ttk.Frame(
            self, relief=RIDGE, border=1, height=10)
        self.Middle_Frame.pack(fill=BOTH, expand=YES)
        self.letter_date_label = ttk.Label(
            self.Middle_Frame, text='NOC Issuance Date', font=('Bell', 12))
        self.letter_date_label.grid(column=0, row=1, padx=10, pady=10)
        self.letter_date_entry = ttk.Entry(
            self.Middle_Frame, width=15, font=('Bell', 12))
        self.letter_date_entry.grid(column=1, row=1, padx=10, pady=10)
        self.letter_to_label = ttk.Label(
            self.Middle_Frame, text='NOC Issued To', font=('Bell', 12))
        self.letter_to_label.grid(column=0, row=2, padx=10, pady=10)
        self.letter_to_text = Text(
            self.Middle_Frame, width=15, height=5, font=('Bell', 12))
        self.letter_to_text.grid(column=1, row=2, padx=10, pady=10)
        self.test_btn = ttk.Button(
            self.Middle_Frame, width=10, command=self.get_to_text, text='Click')
        self.test_btn.grid(column=1, row=3)

    def get_to_text(self):
        print(self.letter_to_text.get(0))
        print(self.letter_to_text.get(1))


filepath = 'F:/Docs/OneDrive/Python Projects/CFC App/Office.db'
Obj = dataentry(filepath)
Obj.mainloop()
